...
if player_subset != 'Choose Player':
    ...
    ptsProp = max(10, roundDownToNearest(thres_pts, 5)) - 0.5
    # rebs base is 2, highest is 18, goes up by 2s
    rebProp = max(2, roundDownToNearest(thres_reb, 2)) - 0.5
    # asts base is 2, highest is 12, goes up by 2s
    astProp = max(2, roundDownToNearest(thres_ast, 2)) - 0.5
    # 3pm base is 1, highest is 7, goes up by 1s
    threesProp = max(1, roundDownToNearest(thres_3pm, 1)) - 0.5
    # pra base is 15, highest is 50, goes up by 5s
    praProp = max(15, roundDownToNearest(thres_pra, 5)) - 0.5
